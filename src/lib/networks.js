...
const Networks = {
  ...
  display: function (config, callback) {
    this.deployed(config, function (err, networks) {
      ...
      if (star_networks.length > 0 && network_names.length > 0 && unknown_networks.length > 0) {
        ...
        config.logger.log(
          OS.EOL +
            "Closely inspect the deployed networks below, and use `tronbox networks --clean` to remove any networks that don't match your configuration. You should not use the wildcard configuration ('*') for staging and production networks for which you intend to deploy your application."
        );
      }
  ...
  matchesNetwork: function (network_id, network_options, callback) {
    ...
    const isSecondANumber = isNaN(parseInt(network_options.network_id)) === false;
