...
var Package = {
  ...
  publish: function(options, callback) {
    ...
    if (!ropsten) {
      return callback(new TruffleError("You need to have a `ropsten` network configured in order to publish to the tronbox Package Registry. See the following link for an example configuration:" + OS.EOL + OS.EOL + "    http://tronboxframework.com/tutorials/using-infura-custom-provider" + OS.EOL));
    }
